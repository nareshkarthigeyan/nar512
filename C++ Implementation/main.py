...
def salt(input):
    ...
    for i in input:
        index ^= ord(i)
        index += not (length | ord(i)) + 88
        index = index % int(sigma(format(ord(i), '08b')), 2) | int(bigM(format(ord(i), '08b')), 2)
    saltIndex = length % ((index * 11) % length - 10)

    for i in range(saltIndex, saltIndex + 7):
